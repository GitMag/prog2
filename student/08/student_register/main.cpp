...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string user_id = parts.at(1);
            if (student_numbers.find(user_id) != student_numbers.end()) {
                ...
                if (is_valid_phone_number(phone_num)) {
                    // valid num, change it
                    student_numbers.at(user_id)->phone_number = phone_num;
                }
                } else {
            ...
            // save data in user id order
            std::ofstream datafile(file_name);

            for (auto pair: user_ids) {
                const Student &s = *pair.second;
                datafile << s.student_number << ";"
                         << s.user_id << ";"
                         << s.name << ";"
                         << s.phone_number << ";"
                         << s.email << ";";
                // write skype if skype is not empty
                if (s.skype != "") {
                    datafile << s.skype;
                }
                datafile << std::endl;
